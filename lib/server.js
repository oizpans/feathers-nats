...
module.exports = function (params = {}) {
  return async function () {
    ...
    for (const service in services) {
      ...
      nats.subscribe(`${name}.${service}.find`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
      ...
      nats.subscribe(`${name}.${service}.get`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
          const [, service, method] = subject.split('.');
          debug(`Request for ${name}.${service}.${method}`);
      ...
      nats.subscribe(`${name}.${service}.create`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
          const [, service, method] = subject.split('.');
          debug(`Request for ${name}.${service}.${method}`);
      ...
      nats.subscribe(`${name}.${service}.patch`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
          const [, service, method] = subject.split('.');
          debug(`Request for ${name}.${service}.${method}`);
      ...
      nats.subscribe(`${name}.${service}.update`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
          const [, service, method] = subject.split('.');
          debug(`Request for ${name}.${service}.${method}`);
      ...
      nats.subscribe(`${name}.${service}.remove`, { queue: name }, async (req, res, subject) => {
        let result = null;
        try {
          const [, service, method] = subject.split('.');
          debug(`Request for ${name}.${service}.${method}`);
